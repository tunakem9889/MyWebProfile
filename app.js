// app.js - neon theme + repo loader + modal + canvas neon background
...
async function loadRepos(){
  ...
  try{
    ...
    // by default show non-forks first (optional)
    repos = data; // or data.filter(r=> !r.fork);
    applyFilterSort();
  }catch(err){
    repoGrid.innerHTML = '';
...
(function neonCanvas(){
  ...
  class Point{
    ...
    draw(){
      ctx.beginPath();
      ctx.fillStyle = `rgba(${this.c.r},${this.c.g},${this.c.b},${this.c.a})`;
  ...
  function frame(){
    ...
    g.addColorStop(0,'rgba(6,6,12,0.65)');
    g.addColorStop(0.5,'rgba(10,6,18,0.45)');
    g.addColorStop(1,'rgba(2,2,8,0.8)');
